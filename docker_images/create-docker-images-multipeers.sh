...
function main() {
    ...
    rm -f ./temp/config/currentView
	
    echo "------------------------------------------------------------------------"
    echo "Creating network configuration and demo scripts"
    echo "------------------------------------------------------------------------"
	#create_hosts_config
	#create_node_config
    create_fabric_orderer_cryptoconfig
    ...
    # note when generating the fabric container, is also generated
    # the fabric configuration where it describes the orderer number of frontends
    # this configuration is based on a sampleconfig dir
    # to customize it, place the configuration at temp/fabric_conf (done by scripts above)
    ...
    # copy the templates and crypto tools to generate a new configuration
    echo "------------------------------------------------------------------------"
    echo "Copying the templates and tools form bftsmart/fabric:$VERSION container"
    echo "------------------------------------------------------------------------"
    ...
    # extract the binaries to generate the credentials:
    echo "------------------------------------------------------------------------"
    echo "Copying the tools form bftsmart/fabric-tools:$VERSION container"
    echo "------------------------------------------------------------------------"
...
function create_fabric_orderer_cryptoconfig () {
#https://raw.githubusercontent.com/wiki/bft-smart/fabric-orderingservice/configuration-material/v1.3/crypto-config.yaml
cat > ./temp/fabric_conf/crypto-config.yaml << 'EOF'
...
Orderer: &OrdererDefaults
    ...
    Addresses:
        ...
        - bft.frontend.3000:7050
        - bft.frontend.4000:7050
...
function create_fabric_network_configtx () {
cat > ./temp/fabric_conf/configtx.yaml << 'EOF'
    ...
    BFTGenesis:
        ...
        Orderer:
            ...
            Addresses:
                ...
                - 3000.frontend.bft:7050
                - 4000.frontend.bft:7050
